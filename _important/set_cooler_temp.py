...
parser.add_argument("-t", "--temp", dest="temp", action="store")
...
try:
    print(C.CanSetCCDTemperature)
    C.SetCCDTemperature = temperature
    log.write(local_time+f" Set CCD Temperature {C.SetCCDTemperature:.2f}\n")
    print(local_time+f" Set CCD Temperature {C.SetCCDTemperature:.2f}")
except Exception as e:
    log.write(local_time+f" FAIL : Set CCD Temperature {temperature:.2f}\n")
    print(local_time+f" FAIL : Set CCD Temperature {temperature:.2f}")
