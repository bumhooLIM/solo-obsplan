...
def run_scheduler():
    ...
    print(f"Note: Tasks will be scheduled 1 local day prior to the UT plan date.")
    
    for i in range(num_days):
        # Calculate UT date (for the filename) and Local date (for Windows Scheduler)
        plan_ut_dt = start_ut_dt + timedelta(days=i)
        local_exec_dt = plan_ut_dt - timedelta(days=1)
        
        file_date_str = plan_ut_dt.strftime("%Y%m%d")
        yaml_filename = f"obsplan_{file_date_str}.yaml"
        yaml_path = obsplan_dir / yaml_filename
        
        print(f"\n[{i+1}/{num_days}] Target Plan: {yaml_filename} (UT: {plan_ut_dt.strftime('%Y-%m-%d')})")
        ...
        if not yaml_path.exists():
            ...
            print(f"   -> Found '{yaml_filename}'.")
        
        # --- 3. Queue in Windows Task Scheduler ---
        task_name = f"SOLO_{file_date_str}"
        task_date = local_exec_dt.strftime("%Y/%m/%d") # Windows requires Local Date
        
        print(f"   -> Queuing Task... (Executes LOCAL: {task_date} at {exec_time})")
